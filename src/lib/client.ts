import { endpoint, type VTSExpression, type VTSHotkey, type VTSModel } from '$lib/config';
import json from '@eslym/svelte-utility-stores/json';
import { sleep, wrapReactive, wrapWritable } from '$lib/utils';
import { derived, get } from 'svelte/store';
import { ApiClient } from 'vtubestudio';
import type { IApiClientOptions } from 'vtubestudio';
import local from '@eslym/svelte-utility-stores/local';
import { Capacitor } from '@capacitor/core';

const icon =
    '';

type IWebSocketLike = ReturnType<Exclude<IApiClientOptions['webSocketFactory'], undefined>>;

let _client: ApiClient;
let _url: string = '';

export const connectionState: {
    connected: boolean;
    authenticated: boolean;
    wsFromHttps: boolean;
} = Object.defineProperties(
    {},
    {
        connected: wrapReactive(false),
        authenticated: wrapReactive(false),
        wsFromHttps: wrapReactive(false)
    }
) as any;

export const vtsState: {
    loadedModel: string | null;
    expressions: VTSExpression[];
    hotkeys: VTSHotkey[];
    models: VTSModel[];
} = Object.defineProperties(
    {},
    {
        loadedModel: wrapWritable(local('vts-current-model')),
        expressions: wrapWritable(json(local('vts-expressions'), () => [])),
        hotkeys: wrapWritable(json(local('vts-hotkeys'), () => [])),
        models: wrapWritable(json(local('vts-models'), () => []))
    }
) as any;

async function refreshExpressions() {
    const e = await _client.expressionState({ details: false });
    vtsState.expressions = e.expressions.map((e) => ({
        name: e.name,
        file: e.file,
        active: e.active
    })) as any;
}

async function refreshModels() {
    const res = await _client.availableModels();
    const models: any[] = [];
    for (const model of res.availableModels) {
        if (model.modelLoaded) {
            vtsState.loadedModel = model.modelID;
        }
        models.push({
            modelID: model.modelID,
            modelName: model.modelName
        });
    }
    vtsState.models = models;
}

async function refreshHotkeys() {
    const res = await _client.hotkeysInCurrentModel({});
    vtsState.hotkeys = res.availableHotkeys.map((k) => ({
        hotkeyID: k.hotkeyID,
        name: k.name,
        description: k.description
    })) as any;
}

let shouldPing = false;

function onConnect() {
    connectionState.authenticated = true;

    refreshHotkeys();

    _client.events.modelLoaded.subscribe(async (data) => {
        vtsState.loadedModel = data.modelID;
        await refreshHotkeys();
    }, {});

    _client.events.modelConfigChanged.subscribe(async (ev) => {
        if (!ev.hotkeyConfigChanged) return;
        await refreshHotkeys();
    });

    (async () => {
        while (connectionState.authenticated) {
            await refreshExpressions();
            await sleep(200);
        }
    })();

    (async () => {
        while (shouldPing) {
            try {
                await refreshModels();
                await sleep(2500);
            } catch (_) {
                if (!shouldPing) return;
                reconnect(get(endpoint));
                break;
            }
        }
    })();
}

function onDisconnect() {
    connectionState.connected = false;
    connectionState.authenticated = false;
    shouldPing = false;
}

let WebsocketClass: new (url: string) => IWebSocketLike = WebSocket;

function getPluginName() {
    if (import.meta.env.DEV) {
        return Capacitor.isNativePlatform() ? 'Remote VTS (DEV)' : 'Remote VTS (Web DEV)';
    }
    return Capacitor.isNativePlatform() ? 'Remote VTS' : 'Remote VTS (Web)';
}

const pluginName = getPluginName();

function reconnect(endpoint: string) {
    if (_client) {
        _client.off('connect', onConnect);
        _client.off('disconnect', onDisconnect);
        _client.disconnect();
    }
    connectionState.connected = false;
    connectionState.authenticated = false;
    connectionState.wsFromHttps = false;
    try {
        _url = endpoint;
        _client = new ApiClient({
            pluginName,
            url: _url,
            pluginDeveloper: '0nepeop1e',
            pluginIcon: icon,
            authTokenGetter() {
                return localStorage.getItem('vts-token');
            },
            async authTokenSetter(token) {
                localStorage.setItem('vts-token', token);
            },
            webSocketFactory(url) {
                const socket = new WebsocketClass(url);
                socket.addEventListener('open', () => {
                    connectionState.connected = true;
                });
                return socket;
            }
        });
        _client.on('connect', onConnect);
        _client.on('disconnect', onDisconnect);
        shouldPing = true;
    } catch (e) {
        connectionState.wsFromHttps = true;
        console.error(e);
    }
}

export const client = derived(endpoint, ($endpoint) => {
    if (import.meta.env.SSR) return undefined as any as ApiClient;
    if (_client && _url === $endpoint) return _client!;
    reconnect($endpoint);
    return _client;
});

if (!import.meta.env.SSR) {
    (async () => {
        if (Capacitor.isNativePlatform()) {
            WebsocketClass = (await import('./capacitor')).CapacitorWebsocket;
        }
        client.subscribe(() => {});
    })();
}
